            ...
            try:
                if self.flag == True:
                    ...
                    # else:
                        ...
                        self.chatText.insert(Tkinter.END, '收货地址改不了'.decode('utf-8')+'\n')

                        time.sleep(1.2)
                        theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        self.chatText.insert(Tkinter.END, '客服小哎' + theTime + ' 说：\n')
                        self.chatText.insert(Tkinter.END,'麻烦您看下订单详情页是否有“ 修改 ”的按钮，如有 ，您可点击修改末级地址/联系人/电话号码/送货时间信息，如没有修改按钮，说明订单已不支持修改了呢 ，还请您理解哦~'.decode('utf-8')+'\n')
                        time.sleep(2.1)
                        theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        self.chatText.insert(Tkinter.END, '您 ' + theTime + ' 说：\n')
                        self.chatText.insert(Tkinter.END,'没找到修改，好像不能修改了怎么办'.decode('utf-8')+'\n')

                        time.sleep(0.8)
                        theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        self.chatText.insert(Tkinter.END, '客服小哎' + theTime + ' 说：\n')
                        self.chatText.insert(Tkinter.END,'如果您地址不远的话，建议您后期关注物流信息，届时可以直接和快递员联系即可。'.decode('utf-8')+'\n')

                        time.sleep(0.8)
                        theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        self.chatText.insert(Tkinter.END, '您' + theTime + ' 说：\n')
                        self.chatText.insert(Tkinter.END,'好的。'.decode('utf-8')+'\n')
